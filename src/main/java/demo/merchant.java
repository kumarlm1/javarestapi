...
public class merchant {
 ...
 public Response requestLogin12(String str) {
      ...
      try {
      	 String query = "insert into merchant(name,description) values(?,?)";
			java.sql.PreparedStatement pd= conn.prepareStatement(query);
			pd.setString(1,name);
			pd.setString(2,des);
 ...
 public Response requestLogin1(String str) {
      ...
      try {
	   String query = "select id from merchant";
       ...
       JSONArray s= (JSONArray) json.get("merchant_item") ;
	    for (int i=0; i<s.length(); i++) {
	    	 JSONObject dat = s.getJSONObject(i);
	    	 int ids = dat.getInt("id");
	    	 childs.remove(new Integer(ids));
	    	 try {
	    	  query = "update merchant set description=?,name=? where id = ?";
   ...
   public Response delete(@PathParam("id") int id){
		JSONObject result = new JSONObject();
	   try {
	    String query = "delete from merchant where id =?";
		java.sql.PreparedStatement pd= conn.prepareStatement(query);
		pd.setInt(1, id);
		pd.execute();
